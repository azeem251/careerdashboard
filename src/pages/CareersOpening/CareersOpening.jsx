...
import CareersAddModel from '../../utils/CareersAddModel';
import CareersUpdateModel from '../../utils/CareersUpdateModel';
const CareersOpening = () => {
  ...
  const getArticleAPI = async () => {
    try {
      setIsLoading(true)
      const response = await fetch(`https://mamun-reza-freeshops-backend.vercel.app/api/v1/admin/allCareerOpening/?search=&fromDate=&toDate=&page=${currentPage}&limit=${itemsPerPage}`, {
        method: "get",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("no Careers Data found")
      }
      const result = await response.json()
      console.log("get Careers resonse", result)
      setGetArticleData(result?.data?.docs)
      setTotalArticles(result?.data?.totalDocs || 0)
      console.log("state Careers data", getArticleData)
    } catch (error) {
      console.log("Careers get catch block error", error.message)
    } finally {
      setIsLoading(false)
    }
  }
...
const handleDeleteArticle = async (id) => {
  Swal.fire({
    title: 'Are you sure?',
    text: 'You want to Delete this Careers Opening!',
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`https://mamun-reza-freeshops-backend.vercel.app/api/v1/admin/deleteCareerOpening/${id}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json"
          }
        ...
        if (!response.ok) {
          throw new Error("Failed to Careers Opening");
        }

        Swal.fire('Deleted!', 'Your Careers Opening has been deleted.', 'success');
    ...
    <>
      <div className='Aritical_wrapper All_wrapper'>
        <div className='top_heaindg'>
          <div>
            <h4 className="text-xl font-semibold  mb-0">Career Opening</h4>
          </div>
          <div className='flex items-center gap-3 aritcal_flex'>
            <div onClick={() => setIsModalOpen(!isModalOpen)}>
              <button className='artical_Add_btn'><span><FaPlus /></span>Add new Career Opening</button>
            </div>
            <div className='articale_delete'><span ><FaRegTrashAlt /></span></div>
            <div>
              <span>Career Opening</span>   /  <Link to='/main'>Dashbaord</Link>
            </div>
          ...
          <table className="table table-hover">
            <thead className='bg-black text-white '>
              <tr>
                ...
                <th style={{ borderRight: "1px solid white" }} scope="col">Location</th>
      ...
      {/* add image model */}
      {isModalOpen && <CareersAddModel getArticleAPI={getArticleAPI} onClose={() => setIsModalOpen(false)} />}
     {/* update img model */}
      {isUpdateModalOpen && (
        <CareersUpdateModel
